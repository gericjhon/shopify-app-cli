...
module ShopifyCli
  class UpdateTest < MiniTest::Test
    ...
    def setup
      ...
      redefine_constant(ShopifyCli::Core::Update, :FETCH_HEAD, File.expand_path('.git/FETCH_HEAD', root))
      ShopifyCli::Config.set('autoupdate', 'enabled', true)
    end

    def teardown
      super
      ShopifyCli::Core::Update.instance_variable_set(:@last_update_time, nil)
    end
    ...
    def test_check_now_aborts_when_head_lock_exists
      ...
      ShopifyCli::Core::Update.expects(:exec).never

      io = capture_io do
        assert_raises ShopifyCli::AbortSilent do
          ShopifyCli::Core::Update.check_now(restart_command_after_update: true, ctx: @context)
        end
      end
    ...
    def test_updated_since_last_recorded_without_recording
      FileUtils.touch(ShopifyCli::Core::Update::FETCH_HEAD, mtime: Time.now)

      assert(ShopifyCli::Core::Update.updated_since_last_recorded?, message: "should have been updated")
    end
    ...
    def test_updated_since_last_recorded_with_same_mtime
      ...
      refute(ShopifyCli::Core::Update.updated_since_last_recorded?, message: "should not have been updated")
    end

    def test_updated_since_last_recorded_with_different_mtime
      FileUtils.touch(ShopifyCli::Core::Update::FETCH_HEAD, mtime: Time.now - 10000)
      ShopifyCli::Core::Update.record_last_update_time
      FileUtils.touch(ShopifyCli::Core::Update::FETCH_HEAD, mtime: Time.now)
    ...
    def test_auto_update_doesnt_check_if_fetch_head_not_stale
      ...
      FileUtils.touch(ShopifyCli::Core::Update::FETCH_HEAD, mtime: Time.now - 10)
      ShopifyCli::Core::Update.expects(:check_now).never
    ...
    def test_auto_update_checks_if_fetch_head_stale
      ShopifyCli::Util.expects(:testing?).returns(false)
      FileUtils.touch(ShopifyCli::Core::Update::FETCH_HEAD, mtime: Time.now - 3601)
      ShopifyCli::Core::Update.expects(:check_now).once
    ...
    def test_prompt_for_updates_sets_config
      ...
      ShopifyCli::Core::Update.expects(:auto_update)

      ShopifyCli::Core::Update.prompt_for_updates
    end
    ...
    def test_check_now_completes_but_does_not_restart
      ...
      capture_io do
        ShopifyCli::Core::Update.check_now(restart_command_after_update: false, ctx: @context)
      end
    end

    def test_check_now_completes_and_restarts
      fake_context_for_success
      ShopifyCli::Core::Update.expects(:exec).with($PROGRAM_NAME, *ARGV, is_a(Hash))
